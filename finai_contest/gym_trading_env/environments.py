...
class TradingEnv(gym.Env):
    ...
    def _set_df(self, df: pd.DataFrame):
        ...
        if not isinstance(df.index, pd.DatetimeIndex):
            ...
            if date_col is None:
                raise ValueError(
                    "TradingEnv expects a DatetimeIndex or a 'date'/'Date' column."
                )
            df[date_col] = pd.to_datetime(df[date_col], errors="coerce")
            if df[date_col].isna().any():
                bad = df.loc[df[date_col].isna()]
                raise ValueError(f"Unparseable dates found in '{date_col}':\n{bad.head()}")
            df = df.set_index(date_col)

        # sort by time and drop timezone (SB3/pandas friendly)
        df = df.sort_index()
        if getattr(df.index, "tz", None) is not None:
            df.index = df.index.tz_convert(None)

        # --- set feature/info arrays as before ---
    ...
    def save_asset_memory(self):
        import pandas as pd
        if not hasattr(self, "historical_info"):
            return pd.DataFrame(columns=["date", "account_value"])

        cols = ["date", "portfolio_valuation"]
        try:
            df = pd.DataFrame(self.historical_info[cols], columns=cols)
        except Exception:
            return pd.DataFrame(columns=["date", "account_value"])

        if not df.empty:
            # Convert numpy datetime64 / int to datetime and format
            df["date"] = pd.to_datetime(df["date"], errors="coerce").dt.strftime("%Y-%m-%d")
            df = df.sort_values("date")

        return df.rename(columns={"portfolio_valuation": "account_value"})
    ...
    def save_action_memory(self):
        ...
        # Drop the last row for SB3 convention
        if len(df) > 1:
            df = df.iloc[:-1]

        # Clean and cast actions
        actions = pd.to_numeric(df["position_index"], errors="coerce").fillna(0).astype(int)
        return pd.DataFrame({"date": df["date"].to_numpy(), "actions": actions.to_numpy(dtype=int)})
